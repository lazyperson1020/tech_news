...
class UserPreference(models.Model):
    ...
    def increment_category(self, category_key: str, value: int = 1):
        ...
        if not isinstance(self.categories, dict):
            self.categories = {}
        self.categories[category_key] = int(self.categories.get(category_key, 0)) + int(value)
...
def handle_search_query(sender, instance, created, **kwargs):
    ...
    category_keywords = {
        'ai_ml': ['ai', 'artificial intelligence', 'machine learning', 'neural', 'transformer', 'llm', 'gpt'],
        'blockchain': ['blockchain', 'bitcoin', 'ethereum', 'crypto', 'web3', 'nft', 'defi', 'smart contract'],
        'web_dev': ['web', 'react', 'vue', 'angular', 'javascript', 'html', 'css', 'frontend', 'backend', 'nodejs'],
        'app_dev': ['app', 'mobile', 'ios', 'android', 'flutter', 'react native', 'swift'],
        'cybersecurity': ['security', 'cybersecurity', 'hacking', 'exploit', 'vulnerability', 'penetration', 'encryption'],
